...
%sys1
syms t alpha Dx x(t) x_sub1(t) x_sub2(t) Dx_sub1(t) Dx_sub2(t) Dx_sub3(t)
x(t) = cos(t) + cos(2 * t) + cos(3 * t);
Dx(t) =  - diff(x,t);
x_sub1(t) = cos(t);
...
hold on;
fplot(@(t)x_sub3(t),[0,T],LineStyle="-.",Color='#7CFC00');
hold on;
...
plot(t_inf_sub3,0,'*', Markersize = 10, Color = '#7CFC00');
hold on;
plot(t_inf_sub3,x_sub3(t_inf_sub3),'*' ,Markersize = 10,Color = '#7CFC00');
hold on;
plot(t_inf_sub3 + pi/3,0,'*', Markersize = 10, Color = '#7CFC00');
hold on;
plot(t_inf_sub3 + pi/3,x_sub3(t_inf_sub3 + pi/3),'*' ,Markersize = 10,Color = '#7CFC00');
hold on;
for i = 0:1:numel(t_inf_sub3)
    line([t_inf_sub3 + pi/3 * i ,t_inf_sub3 + pi/2 * 3],[x_sub3(t_inf_sub3 + pi/3 * i),0],LineStyle = ':',Color = '#7CFC00');
    hold on;
end
...
lgd = legend('cos(t) + cos(2t) + cos(3t)', 'derivative','cos(t)','cos(2t)','cos(3t)');
title('Primal Function: cos(t) + cos(2t) + cos(3t)')
